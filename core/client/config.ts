...
export type NormalizedConfig = Required<
  Omit<CoreOptions, "contactToken" | "initialMessages">
> & {
...
export function createConfig(
  ...
  if (
    ...
    options.pollingInterval < MIN_POLLING_INTERVAL
  ) {
  ...
  const normalizedConfig: NormalizedConfig = {
    ...
    user: options.user ?? {},
    soundEffectFiles: Object.assign(
      {},
      DEFAULT_SOUND_EFFECTS,
      options.soundEffectFiles,
    ),
  ...
  const { getSettings, updateSettings } = createSettingsManager(
