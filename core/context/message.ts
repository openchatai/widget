...
export class MessageCtx {
  ...
  sendMessage = async (
    ...
    /* ------------------------------------------------------ */
    const isSending = this.state.getState().isSendingMessage;
    const isAssignedToAI =
      this.sessionCtx.state.getState().session?.assignee.kind === "ai";
    ...
    try {
      ...
      /* ------------------------------------------------------ */
      if (!this.sessionCtx.state.getState().session?.id) {
      ...
      const sessionId = this.sessionCtx.state.getState().session?.id;
      if (!sessionId) return;
      ...
      if (data?.success) {
        ...
        if (botMessage) {
          const prevMessages = this.state.getState().messages;
  ...
  fetchAndSetHistory = async (sessionId: string): Promise<void> => {
    ...
    if (response && response.length > 0) {
      ...
      this.state.setPartial({
        messages: [...this.state.getState().messages, ...newMessages],
      });
