...
const errorToastMessage = errorToast.querySelector('.error-toast-message') as HTMLDivElement;
const errorToastClose = errorToast.querySelector('.error-toast-close') as HTMLButtonElement;
const connectionStatus = document.getElementById('connectionStatus') as HTMLDivElement;
const connectionStatusText = connectionStatus.querySelector('.connection-status-text') as HTMLSpanElement;
const connectionRetryButton = connectionStatus.querySelector('.connection-retry-button') as HTMLButtonElement;
const chatError = document.getElementById('chatError') as HTMLDivElement;
const chatErrorMessage = chatError.querySelector('.chat-error-message') as HTMLDivElement;
const chatErrorRetry = chatError.querySelector('.chat-error-retry') as HTMLButtonElement;
const emptyState = document.getElementById('emptyState') as HTMLDivElement;
const initialQuestions = document.getElementById('initialQuestions') as HTMLDivElement;
...
async function initializeWidget() {
    try {
        ...
        welcomeMessage.textContent = 'Start a conversation with our AI assistant. We\'re here to help!';
        ...
        if (prelude.initialQuestions && prelude.initialQuestions.length > 0) {
            ...
            const newSuggestionChips = initialQuestions.querySelectorAll('.suggestion-chip');
    ...
    if (state.error.hasError) {
        ...
        switch (state.error.code) {
            ...
            case 'NO_ACTIVE_SESSION':
                showConnectionError('Failed to connect to chat service. Please try again.');
                break;
