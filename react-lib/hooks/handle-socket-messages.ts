...
interface Context<SocketMessage = StructuredSocketMessageType> {
  ...
  onVote?: (
    message: Extract<SocketMessage, { type: "vote" }>,
    _ctx: Context<SocketMessage>,
  ) => void;

  onInfo?: (
    message: Extract<SocketMessage, { type: "info" }>,
    _ctx: Context<SocketMessage>,
  ) => void;

  onSessionUpdate?: (
    message: Extract<SocketMessage, { type: "session_update" }>,
    _ctx: Context<SocketMessage>,
  ) => void;

  onOptions?: (
    message: Extract<SocketMessage, { type: "options" }>,
    _ctx: Context<SocketMessage>,
  ) => void;
...
export function handleSocketMessages(
  _ctx: Context<StructuredSocketMessageType>,
) {
