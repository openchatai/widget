...
function _useContact() {
  ...
  const [contact, setContact] = useSyncedState<T | null>(
    `${botToken}:contact:${user.external_id}`,
    null,
    "local",
  );
  const [creatingContactState, createContactAsync] = useAsyncFn(
...
const [useContact, SafeContactProvider] =
  createSafeContext<ReturnType<typeof _useContact>>();
